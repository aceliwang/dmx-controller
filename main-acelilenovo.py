...
# import pynput
import json
# import threading timer https://www.pythonpool.com/python-timer/#Function_timePerf_counter
...
def main():
    for fixture in fixtureTypes:
        with open(f'{fixture}.json') as config:
            eval('{fixture} = json.load(config)')
...
for line in file:
    ...
    command = args.pop(0)
    if command in ('patch', 'p'): # patch
        
    elif command in ('select', 's'): # select
...
def patch(fixture, address, amount):
    # patch 25 lasers at 1/23
    nextAvailableFixtureNo = patch.keys().sort()
...
def setValue(fixture, type, value, blind):
    if type in ('intensity'):
        channel = fixture.mapping[type].channel
        dmxValue = fixture.mapping
        blindDMX.extend([channel, dmxValue])
    if not blind:
        sendDMX()
