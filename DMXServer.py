...
import ctypes
    ...
    def start():
        handle = 0
        status = FT_Open(0, handle) # needs to be ref handle

    def write(handle, data, length):
        ptr 
...
#             byte value; //The value of a subcommand
        ...
        def command_DMX(input):
            dmxCommand = []
            dmxChannelL = []
            dmxValueL = []
            channel = 0
            value = byte()
            dmxCommand = input.split()
...
#             }
        ...
        if len(dmxCommand) % 2 == 0:
            if verbose:
                print('[ERROR] - Modulo exception. dmx command length is not even')
...
#                 value =  byte.Parse(dmxCommand[i*2]); //Value
    ...
    for i in range(1, len(dmxCommand - 1) / 2 + 1):
        channel = int(dmxCommand[i * 2 - 1])
        value = byte(dmxCommand[i * 2])
...
#                 }
        if channel in blockedChannels:
            if verbose:
                print(f'[DEBUG] - Channel {0} is blocked at the moment', channel)
                continue
...
#                 dmxValueL.Add(value);
            if channel < 0 or channel > 513 or value < 0 or value > 255:
                # throw new Exception
            dmxChannelL.append(channel)
            dmxValueL.append(value)
...
#             }
    ...
    for i, j in enumerate(dmxChannelL):
        if verbose:
            print(f'[DEBUG] - Setting: {0}', i)
            OpenDMX.setDmxValue()
...
#             effectCommand.RemoveAt(0); //get rid of the name in the list
        ...
        effectCommand = [i for i in input[7].split(' ')]
        effectName = effectCommand.pop(0)
...
#             }
        ...
        if len(effectCommand) % 3 != 0 or effectName in activeEffects.keys():
            print('Modulo or effectName exception')
            raise ValueError('')
...
class OpenDMX:
    buffer = bytearray()
    handle = '' # can this not be declared as needed
    done = False
    bytesWritten = 0
    # create FT_STATUS data type?
...
#         public static extern FT_STATUS FT_SetDivisor(uint ftHandle, char usDivisor);
    ...
    def FT_Open(uiPort, ftHandle):
        ftd2xx.FT_Open(uiPort, ftHandle)
...
#         }
    ...
    def start():
        handle = 0
        status = FT_Open(0, handle) # TODO: needs to pass ref, not value. Change handle into a function?
...
#         }
    ...
    def writeData():
        while not done:
            ...
            FT_SetBreakOn(handle)
            FT_SetBreakOff(handle)
            bytesWritten = write(handle, buffer, len(buffer))
            Thread.Sleep(tickSpeed) # TODO
...
#         }
    ...
    def write(handle, data, length):
        ptr = 
        bytesWritten = 0
        status = FT_Write(handle, ptr, length, bytesWritten) # TODO: bytesWritten is a ref
        return bytesWritten
...
#         }
    ...
    def initOpenDMX():
        ...
        status = FT_SetDataCharacteristics(handle, BITS_8, STOP_BITS_2, PARITY_NONE)
        status = FT_SetFlowControl(handle, FLOW_NONE, 0, 0) # TODO: check if typing (CHAR) changes anything
        status = FT_ClrRts(handle)
        status = FT_Purge(handle, PURGE_TX)
        status = FT_Purge(handle, PURGE_RX)
...
#     };
    ...
    class FT_STATUS(enum.Enum): # TODO: this doesn't work in python does it
        FT_OK = 0
        FT_INVALID_HANDLE
        FT_DEVICE_NOT_FOUND
        FT_DEVICE_NOT_OPENED
